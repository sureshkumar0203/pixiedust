...
.table thead th {
	vertical-align: bottom;/* border-bottom: 1px solid #000;*/
            }
.table td, .table th {
 padding: .75rem;
	vertical-align: top;
...
<div class="page">
  <div class="subpage">
    <table width="100%" border="0" cellpadding="0" cellspacing="0">
      <tr>
        <td>
          <table width="100%" cellpadding="5"  border="1" id="section_a2" style="border:solid 1px #00000024; border-collapse:collapse; font-size:12px;">
              <tr>
              <td colspan="3" align="left" valign="middle" style="padding-top:30px;"><img src="{{ asset('public/images/admin-logo.png') }}" width="120" alt=""/> <span style="float:right; line-height: 80px; padding-right:20px;">Date:{{ date('d-m-Y') }}</span></td>
            </tr>
              <tr style="font-size:12px; line-height:15px;">
              <td width="33%" height="40" align="left" valign="top"> From
                  <address>
                <strong>PixieDust</strong><br>
                {{ strip_tags(getAdminDetails()->address) }} <br>
                Phone: {{ getAdminDetails()->contact_no }}<br>
                Email: {{ getAdminDetails()->alt_email }}
                </address></td>
              <td width="33%" align="left" valign="top"> To
                  <address>
                ...
                City : {{ $getOrderDetails->ship_city }},State : {{ $getOrderDetails->ship_state }}<br>
                Country : {{ $getOrderDetails->ship_country }} Pincode: {{ $getOrderDetails->bill_post_code }}<br>
                Phone: {{ $getOrderDetails->ship_phone_number }}<br>
                Email: {{ $getOrderDetails->email }}
                </address></td>
              <td width="33%" align="left" valign="top"><b>Invoice #pixie{{ $getOrderDetails->id }}</b><br>
                  <br>
                  <b>Order ID:</b> {{ $getOrderDetails->id }}<br></td>
            </tr>
            </table>
        </td>
      </tr>
      ...
      <tr>
        <td><table border="0" cellpadding="0" cellspacing="0" class="table table-striped">
            <thead>
            <tr>
                <th align="left" valign="middle">SL No.</th>
                <th align="left" valign="middle">Product Details</th>
            ...
            <td align="left" valign="middle" height="40"> {{ getProductDetails($orderItem->product_id)->name }}<br>
                <span> @if($orderItem->size) Size :{{ $orderItem->size }} @endif</span><br>
                <span> @if($orderItem->color) {{ $orderItem->color }} @endif </span> 
                </td>
      ...
      <tr>
        <td align="right" valign="top" style="border-top:solid 1px #ccc;text-align:right;"><table border="0" class="table" style="width:300px; float:right;">
            <tbody>
            <tr>
            ...
            <tr>
            <th align="left" valign="middle" class="total">Tax</th>
            <td align="left" valign="middle" class="total total2">${{ $getOrderDetails->tax_amount ? $getOrderDetails->tax_amount:"0.00" }}</td>
          </tr>
            @endif
            <tr>
